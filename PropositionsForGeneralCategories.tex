\subsection{Propositions for general categories}
...
\begin{sequent}
\begin{align*}
  L: \List ~|~ \big(\forall x \in L: \mathrm{IsTerminal}(x)\big) \vdash \mathrm{IsTerminal}\big( \mathrm{DirectProduct}( L ) \big)
\end{align*}
\end{sequent}

\begin{sequent}
\begin{align*}
  L: \List ~|~ \big(\forall x \in L: \mathrm{IsInitial}(x)\big) \vdash \mathrm{IsInitial}\big( \mathrm{Coproduct}( L ) \big)
\end{align*}
\end{sequent}